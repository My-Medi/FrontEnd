...
interface ExpertGageProps {
  userId: number;
  round: number;
  nickname: string;
}

const ExpertGage: React.FC<ExpertGageProps> = ({ userId, round, nickname }) => {
  const { data: resultData, isLoading, error } = useExpertUserReportResultQuery(userId, round);
  ...
  const { totalScore, healthStatus } = resultData;
