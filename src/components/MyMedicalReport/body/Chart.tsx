...
const Chart: React.FC<ChartProps> = ({ checkupDate, nickname }) => {
  // const [selectedCategory, setSelectedCategory] = useState<Category>('비만/복부비만');
  // 1. patientIndicators 객체를 배열로 펼침
  const rawIndicators = Object.values(patientIndicators).flat();

  // 2. level -> stage로 키 변환해서 Gage에 맞게 가공
  const indicators = rawIndicators.map((item) => ({
    id: item.id,
    stage: item.level, // level 키를 stage로 맞춰줌
  }));
  return (
    <div className='w-[1301px] max-h-[2364px] flex-shrink-0 rounded-[20px] border-[2px] border-[#DBE6FF] bg-[linear-gradient(157deg,_rgba(161,189,255,0.30)_-0.5%,_rgba(219,230,255,0.30)_85.34%)] flex flex-col items-center pt-[46px] relative'>
      {/* 회색 클립 이미지 그룹 */}
      ...
      {/* 내부 흰색 박스 */}
      <div className='w-[1237px] max-h-[2272px] mt-[10px] mb-[46px] flex-shrink-0 rounded-[20px] bg-white shadow-[0_2px_4px_rgba(119,119,119,0.25)] px-[70px] pt-[60px] pb-[40px] relative z-20'>
        {/* 왼쪽 작은 클립 */}
        <img
          src={ChartSideClip}
          alt='left-clip'
        ...
        <div className='mb-10 flex flex-col items-center'>
          <div className='text-[24px] font-semibold leading-[36px] tracking-[-0.72px] text-[#121218] flex items-center justify-center gap-[8px]'>
            <div className='flex items-center gap-[16px] ml-[-280px] mr-[100px]'>
              <div className='text-[#25282B] text-[20px] font-medium tracking-[-0.6px]'>검진일</div>
