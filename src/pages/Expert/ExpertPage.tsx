...
const mapExpertToDetail = (expert: Expert): ExpertDetail => ({
  ...
  profileImage: (expert as any).profileImgUrl || (expert as any).profileImageUrl || '', // 우선순위: profileImgUrl > profileImageUrl > profile
...
const ExpertPage = () => {
  ...
  const getExpertStatus = (expertId: number): 'matched' | 'request' | 'rejected' => {
    ...
    const matchedExpert = matchedExpertsData.result.find((expert) => expert.expertId === expertId);
  ...
  const actualTotalPages = pagedList.length === 0 && currentPage > 1 ? currentPage - 1 : totalPages;
              ...
              {pagedList.map((expert: Expert) => {
                return (
                  <ExpertCard
                    ...
                    profile={(expert as any).profileImgUrl || (expert as any).profileImageUrl || expert.profile || ''}
                    careers={[]}
